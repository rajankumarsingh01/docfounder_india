...
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    draw();
    const onResize = () => { W = canvas.width = window.innerWidth; H = canvas.height = 520; };
    window.addEventListener("resize", onResize);
    return () => { cancelAnimationFrame(raf); window.removeEventListener("resize", onResize); };
  }, []);
  return <canvas ref={canvasRef} className="hero-particles" />;
        ...
        {!done ? (
          <>
            <div className="claim-modal-header">
              <div className="claim-doc-icon-wrap">
                <div className="claim-doc-icon">
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                    <path d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z" stroke="#e8352a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    <path d="M14 2V8H20M16 13H8M16 17H8M10 9H8" stroke="#e8352a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
                <div className="claim-doc-icon-ring" />
              </div>
            ...
            <div className="claim-form">
              ...
              <div className="form-group">
                <label className="form-label"><span className="label-dot" />Why is this your document?<span className="label-req">*</span></label>
                <textarea className="form-input form-textarea" name="reason" value={form.reason} onChange={handleChange} placeholder="Describe identifying details..." rows={4} />
              </div>
              <div className="form-group">
                <label className="form-label"><span className="label-dot" />Your Contact<span className="label-opt">(optional)</span></label>
                <input className="form-input" name="contact" value={form.contact} onChange={handleChange} placeholder="Phone or email" />
              </div>
...
function DocCard({ doc, onClaim, index }) {
  ...
  const handleMouseLeave = () => {
    if (cardRef.current) cardRef.current.style.transform = "perspective(900px) rotateX(0deg) rotateY(0deg) translateY(0) scale(1)";
  };

  return (
    <div className="search-card" ref={cardRef} style={{ animationDelay: `${index * 0.07}s` }} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
      <div className="sc-glow" />
      <div className="sc-image-wrap">
        {doc.imageUrl
          ? <img src={doc.imageUrl} alt="Document" className="sc-image" />
          : <div className="sc-placeholder"><div className="sc-placeholder-inner"><svg width="38" height="46" viewBox="0 0 38 46" fill="none"><rect x="1" y="1" width="36" height="44" rx="3" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5"/></svg></div><div className="sc-placeholder-label">{doc.documentType}</div></div>
        }
        <div className="sc-image-overlay" />
        <span className={`sc-status ${status.color}`}><span className="sc-status-dot" />{status.label}</span>
        <div className="sc-date-chip">{new Date(doc.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
      </div>
      <div className="sc-body">
        <h3 className="sc-type">{doc.documentType}</h3>
        <div className="sc-metas">
          <div className="sc-meta-row"><span className="sc-meta-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></span><span className="sc-meta-val">{doc.partialName}</span></div>
          <div className="sc-meta-row"><span className="sc-meta-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 1118 0z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><circle cx="12" cy="10" r="3" stroke="currentColor" strokeWidth="1.8"/></svg></span><span className="sc-meta-val">{doc.foundLocation}</span></div>
        </div>
        {doc.finderContact && (
          <div className="sc-revealed-contact">
            <div className="sc-reveal-label">Finder's Contact</div>
            <span className="sc-reveal-val">{doc.finderContact}</span>
          </div>
        )}
        <div className="sc-actions">
          {canClaim && <button className="sc-btn-claim" onClick={() => onClaim(doc)}>This is Mine <span className="btn-claim-arrow">→</span></button>}
          {doc.status === "PENDING_CLAIM" && !isOwner && <div className="sc-status-pill sc-pending-note"><span className="pill-dot pending-dot" />Claim under review</div>}
          {isOwner && <button className="sc-btn-edit" onClick={() => navigate(`/documents/edit/${doc.id}`)}>My Upload</button>}
          {doc.status === "CLAIMED" && <div className="sc-status-pill sc-claimed-note">Document Returned</div>}
          {!user && doc.status === "FOUND" && <button className="sc-btn-claim sc-btn-signin" onClick={() => navigate("/login")}>Sign in to Claim →</button>}
        </div>
      </div>
    </div>
  );
...
function SearchDocumentsPage() {
  ...
  useEffect(() => {
    const af = [];
    if (filters.documentType) af.push({ key: "documentType", label: filters.documentType });
    if (filters.location)     af.push({ key: "location",     label: `📍 ${filters.location}` });
    if (filters.name)         af.push({ key: "name",         label: `👤 ${filters.name}` });
    setActiveFilters(af);
  }, [filters]);
  ...
  const fetchDocuments = async (f = {}, page = 1) => {
    setLoading(true);
    try {
      const res = await api.get("/documents", { params: { ...f, page, limit: 9 } });
  ...
  return (
    <div className="search-page">
      ...
      <section className="search-hero">
        ...
        <ParticleCanvas />
        <div className="hero-content">
          <div className="hero-eyebrow"><span className="eyebrow-dot" />India's #1 Lost Document Platform</div>
          <h1 className="hero-title">Find Your<br /><span className="hero-accent">Lost Document</span></h1>
          <p className="hero-sub">Search among documents found across India. If yours is here — claim it, contact the finder, get it back.</p>
          <div className="hero-stats">
            <div className="hero-stat"><span className="stat-num">2,400+</span><span className="stat-label">Docs Found</span></div>
            <div className="hero-stat-divider" />
            <div className="hero-stat"><span className="stat-num">1,800+</span><span className="stat-label">Reunited</span></div>
            <div className="hero-stat-divider" />
            <div className="hero-stat"><span className="stat-num">500+</span><span className="stat-label">Cities</span></div>
          </div>
          <form className="search-form" onSubmit={handleSearch}>
            <div className="search-row">
              <div className="search-field-wrap">
                <span className="search-field-icon"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/></svg></span>
                <select name="documentType" className="search-select" value={filters.documentType} onChange={handleFilterChange}>
                  <option value="">All Document Types</option>
                  {DOCUMENT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div className="search-field-wrap">
                <span className="search-field-icon"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 1118 0z" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/><circle cx="12" cy="10" r="3" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/></svg></span>
                <input type="text" name="location" className="search-input" placeholder="Location — Delhi, Pune…" value={filters.location} onChange={handleFilterChange} />
              </div>
              <div className="search-field-wrap">
                <span className="search-field-icon"><svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5"/></svg></span>
                <input type="text" name="name" className="search-input" placeholder="Name on document (partial)" value={filters.name} onChange={handleFilterChange} />
              </div>
            </div>
            <div className="search-btns">
              <button type="submit" className="btn-search" disabled={loading}>
                {loading
                  ...
                  : <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{marginRight:8}}><circle cx="11" cy="11" r="8" stroke="currentColor" strokeWidth="2"/><path d="m21 21-4.35-4.35" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Search Documents</>
                }
              </button>
              <button type="button" className="btn-reset" onClick={handleReset}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" style={{marginRight:6}}><path d="M3 12a9 9 0 109-9H3M3 12l4-4M3 12l4 4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>Reset
              </button>
            </div>
          </form>
        </div>
              ...
              <div className="filter-chips">
                {activeFilters.map((f) => (
                  <span key={f.key} className="filter-chip">
                    {f.label}
                    <button className="chip-remove" onClick={() => removeFilter(f.key)}>✕</button>
                  </span>
                ))}
              </div>
            )}
